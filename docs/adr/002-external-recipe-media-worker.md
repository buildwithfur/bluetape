...
For social video, parse the title plus caption/description first. If that evidence already contains a useful ingredient list and actionable steps, return the draft without following links or downloading media. If it is insufficient, try up to three distinct public, non-social HTTP(S) links embedded in the caption/description in source order; apply the website extraction path to each and retain the original social post as recipe provenance. A linked page wins only when it yields a complete recipe on its own, because a caption often links a blog root rather than the specific post; a partial match falls through to the media evidence instead. This linked-page step uses the same SSRF, redirect, response-size, and proxy protections as a direct website import, but its proxy activation stays scoped to the linked host so a blocked third-party page cannot push the social downloads onto the proxy. If no linked page yields a recipe, fetch original-language subtitles without downloading the video and parse the accumulated evidence again. Download and transcribe the smallest suitable audio stream only when the subtitle-enriched draft is still incomplete. Download a 480p-or-smaller video and inspect a bounded set of sampled frames only when the transcript still leaves visual gaps such as on-screen quantities or unstated actions. Website imports do not use the media binaries: fetch bounded HTML, prefer Recipe JSON-LD/schema.org, and fall back to readable page text plus structured LLM extraction.
