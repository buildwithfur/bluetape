...
- The family language is implicit rather than stored: it is the family owner's profile locale. Recipe detail may request that owner locale as a second target and shows the owner-language ingredient beneath the viewer translation when the locales differ and both values are available.
...
15. **i18n has separate UI and user-content layers.** UI strings ship through `react-i18next` in English, Indonesian, and Burmese. Authored fields record the profile locale used when each field is created or edited. Supported user-content fields use an operator-gated, on-demand `contentTranslations` cache keyed by exact source hash and target locale; a job is created only when the recorded source locale differs from the requested target. Authored source remains authoritative and visible as the fallback. Legacy fields without locale metadata retain provider-detection fallback behavior. Tasks are the first merged entity and recipes reuse that lifecycle for title, ingredients, and steps. Within a family, the owner's profile locale is the implicit family language; recipe detail can show owner-language ingredients beneath viewer translations when the locales differ. See §6.10.
...
29. **Recipes are structured first-class records, not generic markdown pages.** Import creates a persistent draft, preserves the source, and requires a review of extracted ingredients + ordered steps before publish. Social sources first use caption/description, then follow a bounded set of public non-social recipe links from that text when it lacks a usable recipe, and only then fall back to transcript/media evidence. Websites prefer Recipe schema and fall back to LLM extraction. A linked-page import retains the original social post as its source provenance.
